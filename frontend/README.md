...
- Benchmark & Scan **Trend Analytics** (History tab) loads segmented rollups via `GET /benchmarks/analytics/trends` and `GET /scans/analytics/trends` (window/bucket hours + environment/suite/type/agent/status), reusing History Filters for agent/env/suite/type scope.
