...
| RSK-018 | Residual orchestration scope: trigger nodes and parallel fork/join edge cases may still fall back to stub output when live executor is enabled; operators must validate graphs before unattended prod automation. | Medium | Medium | Low | Security Architecture + Platform Engineering | 2026-08-15 | Partial — live executor covers primary gateway nodes; accepted with validate + approval gates |
| RSK-019 | Flow Orchestration live notification delivery may enable spam or misdirected alerts without per-channel rate limits; domain allowlist is enforced at send time but volume controls remain open. | Medium | Medium | Medium | Security Architecture + Trust Operations | 2026-08-30 | Partial — live SendGrid/Twilio/webhook delivery shipped with allowlist + binding-only secrets; rate limits deferred |
| RSK-020 | Orchestration IGA JIT grants temporarily expand flow run/approve/manage access; mis-approval or stale certification attestation increases standing-privilege risk until expiry. | Medium | Medium | Low | Security Architecture + IAM Governance | 2026-09-15 | Partial — time-bound JIT, SoD defaults in prod, certification gate on prod run, approval event trail |
...
| CC-031 | Flow Orchestration constrains workflows via role-gated CRUD/run/approve, inline-secret rejection, HTTP host allowlist default deny (`orchestration.http_allowed_hosts_json`), max node cap, prod approval gate, dual approval on prod approve and prod runs with human_approval nodes, rate limits on mutations, audit events for allow/deny paths, and opt-in live executor for gateway-backed nodes including HTTP auth bindings, email/SMS delivery, and human approval pause/resume. Residual: trigger/passthrough stub fallback and prod flag gating (RSK-018 partial). | RSK-018 | Preventive/Detective | `/orchestration/*`, `orchestration.flow.*` audit events, `test_orchestration_flows.py`, `flow-orchestration-impact-analysis.md` |
| CC-033 | Flow Orchestration IGA enforces SoD on approve (`orchestration_iga.enforce_sod_on_approve`), staged approval state, JIT time-bound grants, access policy certification before prod run (`orchestration.prod_run_requires_access_certification`), optional `GatewayEntitlement` linkage, immutable `OrchestrationFlowApprovalEvent` records, and explain/posture APIs. Residual: JIT list/due-cert UI gaps (RSK-020 partial). | RSK-020 | Preventive/Detective | `/orchestration/flows/*/iga/*`, `/orchestration/jit-access-requests*`, `/orchestration/access-certifications/due`, `test_orchestration_iga.py`, `flow-orchestration-iga-impact-analysis.md` (GOV-FLOW-IGA-001) |
| CC-032 | Flow Orchestration notification channels constrain email/SMS workflows via registry validation (`gateway.notification_channels_json`, no inline secrets, `credential_binding_id` required when enabled), sensitive-key dual approval on registry mutation, flow validation requiring enabled channel + binding, recipient template secret-pattern rejection, runtime domain allowlist enforcement on live send, delivery via `gateway_notification_delivery`, and audit on channel context read. Residual: per-channel rate limits (RSK-019 partial). | RSK-019 | Preventive/Detective | `GET /gateway/notification-channels*`, `email_send`/`sms_send` nodes, `gateway.notification_channel.context.read`, `test_gateway_notification_delivery.py`, `flow-orchestration-notification-impact-analysis.md` (GOV-FLOW-NOTIFY-001) |
