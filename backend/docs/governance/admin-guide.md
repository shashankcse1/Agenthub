...
## 10. Directory Account Lockout Governance

- Treat account unlock as a privileged action requiring MFA-backed admin execution.
- Use `POST /auth/directory/users/{user_id}/unlock` only after ticketed operator review.
- Periodically review runtime lockout policy values:
	- `auth.login.max_failed_attempts`
	- `auth.login.lockout_minutes`
- Review unlock audit events (`auth.directory.user.unlock`) for potential misuse patterns.

## 11. Directory Identity Lifecycle Governance

- Ensure directory user, group, and team creation/update/delete operations follow approved admin workflow.
- Verify membership changes (group/team) are ticketed and traceable.
- Confirm user provisioning includes compliant password setup and rotation handling.
- Periodically review directory identity changes against audit records for least-privilege drift.
